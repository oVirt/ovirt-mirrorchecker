...
RUN yum install -y centos-release-scl && yum install -y rh-python35 libffi-devel \
      python-devel openssl-devel nss_wrapper gettext gcc git && yum clean all -y
RUN ["scl", "enable", "rh-python35", "pip install setuptools==18.5 --no-cache-dir --process-dependency-links --allow-all-external"]
RUN ["scl", "enable", "rh-python35", "pip install git+https://gerrit.ovirt.org/mirrorchecker --no-cache-dir --process-dependency-links --allow-all-external"]
